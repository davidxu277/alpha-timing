...
# ── ablation: is the regime classifier redundant? ─────────────────────────────
def _spy_frame(clf, scaler):
    """Build the SPY test slice (features + market returns), shared by ablation variants."""
    spy = download_spy()
    kg = pd.read_csv(DATA, parse_dates=["date"])
    macro = kg[kg["ticker"] == "^GSPC"][["date", "vix", "fed_funds_rate",
                                         "unemployment_rate", "10y_treasury", "2y_treasury"]]
    spy = spy.merge(macro, on="date", how="inner").sort_values("date").reset_index(drop=True)
    ts = spy["date"].iloc[int(len(spy) * 0.8)]
    sf = pd.concat([spy[["date", "close"]], build_factors(spy["close"]),
                    classifier_features(spy)], axis=1).dropna(subset=FACTORS + CLF_FEATURES)
    sf = sf[sf["date"] >= ts].reset_index(drop=True)
    return sf, clf.predict(sf[CLF_FEATURES]), scaler.transform(sf[FACTORS])


def _stats(pos, mkt):
    r = pos * mkt + (1 - pos) * CASH_D - COST * np.abs(np.diff(np.concatenate([[0.0], pos])))
    nav = np.cumprod(1 + r)
    return (nav[-1] - 1, np.sqrt(252) * r.mean() / r.std(),
            (nav / np.maximum.accumulate(nav) - 1).min(), float(np.mean(pos > 0)))


def ablation() -> None:
    """Remove the classifier and re-measure, to prove routing is not redundant.

      A  full system:      4 regime specialists + regime-asymmetric thresholds
      C  no expert routing: single pooled GBT + regime-asymmetric thresholds
      B  no classifier:     single pooled GBT + one symmetric threshold pair
    """
    print("preparing + training full stack ...")
    df = prepare()
    clf, scaler, spec = train_stack(df)
    th_on, th_off = select_thresholds(df)

    print("training the single pooled model (all regimes together) ...")
    train = df[df["date"] < TRAIN_END]
    single = HistGradientBoostingRegressor(max_depth=3, learning_rate=0.05,
                                           max_iter=300, random_state=7)
    single.fit(scaler.transform(train[FACTORS]), train["y_fwd"])
    df["yhat_single"] = single.predict(scaler.transform(df[FACTORS]))

    # validation slices (2013-2018) for selecting single-model thresholds
    def val_slices():
        out = {}
        for tk, g in df.groupby("ticker"):
            g = g[(g["date"] >= VAL[0]) & (g["date"] < VAL[1])]
            out[tk] = {"mkt": g["mkt_1d"].to_numpy(), "rf": g["rf_d"].to_numpy(),
                       "yhat": g["yhat_single"].to_numpy(), "date": g["date"].to_numpy(),
                       "riskoff": g["regime_pred"].isin(RISK_OFF).to_numpy(),
                       "vs": np.clip(VOL_TARGET_D / np.maximum(g["vol_20"].to_numpy(), 1e-6), 0, 1)}
        return out

    def val_calmar(sl, pos_fn):
        frames = []
        for s in sl.values():
            p = pos_fn(s)
            r = p * s["mkt"] + (1 - p) * s["rf"] - COST * np.abs(np.diff(np.concatenate([[0.0], p])))
        ...
        return (nav[-1] ** (252 / len(ret)) - 1) / abs(mdd) if mdd < 0 else 99.0

    sl = val_slices()
    # B: single symmetric threshold pair
    bestB = None
    for ti in GRID_IN:
        for to in GRID_OUT:
            sc = val_calmar(sl, lambda s, ti=ti, to=to:
                            hysteresis(s["yhat"], np.full(len(s["yhat"]), ti),
                                       np.full(len(s["yhat"]), to)) * s["vs"])
            if bestB is None or sc > bestB[0]:
                bestB = (sc, ti, to)
    _, B_in, B_out = bestB
    # C: single model + regime-asymmetric thresholds
    bestC = None
    for a in GRID_IN:
        for b in GRID_OUT:
            for c in GRID_IN:
                for d in GRID_OUT:
                    def pf(s, a=a, b=b, c=c, d=d):
                        tin = np.where(s["riskoff"], c, a)
                        tout = np.where(s["riskoff"], d, b)
                        return hysteresis(s["yhat"], tin, tout) * s["vs"]
                    sc = val_calmar(sl, pf)
    ...
    # apply to SPY test
    sf, regp, X = _spy_frame(clf, scaler)
    c = sf["close"].to_numpy()
    mkt = np.append(c[1:] / c[:-1] - 1, 0)
    riskoff = np.isin(regp, list(RISK_OFF))
    vs = np.clip(VOL_TARGET_D / np.maximum(sf["vol_20"].to_numpy(), 1e-6), 0, 1)
    yhat4 = np.zeros(len(sf))
    for r in REGIMES:
        m = regp == r
        if m.any():
            yhat4[m] = spec[r].predict(X[m])
    yhat1 = single.predict(X)

    A = _stats(system_position(yhat4, riskoff, vs, th_on, th_off), mkt)
    tinC = np.where(riskoff, C_off[0], C_on[0]); toutC = np.where(riskoff, C_off[1], C_on[1])
    C = _stats(hysteresis(yhat1, tinC, toutC) * vs, mkt)
    B = _stats(hysteresis(yhat1, np.full(len(sf), B_in), np.full(len(sf), B_out)) * vs, mkt)
    BH = _stats(np.ones(len(sf)), mkt)

    print("\n=== ABLATION (SPY test, 5 bps) — does the regime classifier matter? ===")
    print(f"{'variant':<48}{'total':>8}{'Sharpe':>8}{'maxDD':>8}{'TIM':>7}")
    print("-" * 79)
    for name, m in [("A  full system (4 specialists + regime thresholds)", A),
                    ("C  single model + regime thresholds (no routing)", C),
                    ("B  single model + one threshold (no classifier)", B),
                    ("   Buy & Hold", BH)]:
        print(f"{name:<48}{m[0]:+8.1%}{m[1]:8.2f}{m[2]:8.1%}{m[3]:7.0%}")
