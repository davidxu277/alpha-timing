...
**怎么读这些建议。** 系统的仓位是一个 **0~100% 的连续数**(由波动目标那层缩放),不是"满仓/空仓"两档。看板直接按"今天仓位相对昨天往哪个**方向**动"给出建议:

| 建议 | 仓位方向 | 涵盖 |
|---|---|---|
| **BUY 买入** | 变大 | 建仓(从空仓进场),**或**波动下降时加仓 |
| **SELL 卖出** | 变小 | 清仓(离场到现金),**或**波动上升时减仓 |
| **HOLD 持有** | 基本不变 | 满仓持有,**或**空仓观望 |

进场 / 离场由**双门槛滞后**(见亮点 2)跨越门槛带决定;更细的加仓 / 减仓来自**波动目标**(见亮点 1,波动下降 → 加,波动上升 → 减)。小于 3% 的变动算作"没动"。
