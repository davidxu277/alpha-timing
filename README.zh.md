...
整套系统建立在市场分析里最古老的一条思想上:**"历史会重演"(history repeats itself)**。它是**技术分析三大前提**之一 [[1]](#参考文献)(另两条:市场消化一切信息、价格沿趋势运行),源头是**道氏理论(Dow Theory)** [[2]](#参考文献)。其逻辑是行为层面的,而非玄学:市场结果由人的决策产生,而人在恐惧与贪婪下的心理足够稳定,以至于**相似的环境往往带来相似的结果**,从而留下可反复识别的模式。

这正是本系统所做的事。梯度提升专家从不靠公式预测——每个预测就是"与今天最相似的历史交易日的平均未来收益"(见 [状态专家](#2--状态专家))。分类器则把"相似环境"精确化:把每一天归入少数几个**反复出现的市场状态**——这就是同一思想在计量经济学里可检验的形式,即**状态转换(regime-switching)** [[3]](#参考文献)。

一个诚实的说明(与本文其余部分一致):**"历史重演"是一个工作假设,而非定律**。它与有效市场假说长期处于张力之中,而这里的超额收益本身就薄、且由事件驱动(见 [设计取舍](#设计取舍))。这套系统是一次有纪律的下注——**赌"历史押韵的次数足够多,值得去交易"**,而不是断言它"必然"重演。
...
**怎么读这些建议。** 系统的仓位是一个 **0~100% 的连续数**(由波动目标那层缩放),不是"满仓/空仓"两档。所以除了买/持/卖,看板会按"今天仓位相对昨天怎么变"给出更细的建议:

| 建议 | 触发条件 | 对应经典说法 |
|---|---|---|
| **BUY 买入** | 昨天 0 仓 → 今天有仓(跨过**进场**线) | 买入 |
| **SELL 卖出** | 昨天有仓 → 今天 0 仓(跌破**离场**线) | 卖出 |
| **CASH 空仓** | 一直 0 仓,继续观望 | 持币不动 |
| **ADD 加仓** | 仍持仓,但仓位比昨天**涨了 >3%** | 半个"买入" |
| **TRIM 减仓** | 仍持仓,但仓位比昨天**跌了 >3%** | 半个"卖出" |
| **HOLD 持有** | 仍持仓,仓位基本没变 | 持有 |

**BUY / SELL** 由**双门槛滞后**(见亮点 2)决定,是 0 仓与有仓之间的"进场 / 离场";**ADD / TRIM** 则是在持仓状态下、由**波动目标**(见亮点 1)重新缩放仓位(波动下降 → 加仓,波动上升 → 减仓);3% 就是区分"微调"和"没动"的门槛。
...
底层前提(见 [底层前提](#底层前提--为什么学历史能管用))与建模思路参考自:

1. Murphy, J. J. (1999). *Technical Analysis of the Financial Markets*. New York Institute of Finance. —— 技术分析三大前提,含"历史会重演"。
2. Rhea, R. (1932). *The Dow Theory*. Barron's. —— 对查尔斯·道社论的经典系统化,此前提由此而来。
3. Hamilton, J. D. (1989). A New Approach to the Economic Analysis of Nonstationary Time Series and the Business Cycle. *Econometrica*, 57(2), 357–384. —— 状态转换:市场在少数反复出现的状态间循环。

*("历史不会重复,但常会押韵" 一句流行地归于马克·吐温,但该出处未经证实。)*
